...
class BidCarrousel(ttk.Frame):
    def __init__(self, parent, bid_dir="bid", callback=None):
        ...
        self.thumbnail_size = 80  # Reduced size to accommodate 8 thumbnails
    ...
    def load_bids(self):
        ...
        for f in os.listdir(self.bid_dir):
            if f.endswith('.bid'):
                ...
                mtime = os.path.getmtime(path)
                self.bid_files.append((f, mtime))
        
        # Sort by modification date (most recent first)
        self.bid_files.sort(key=lambda x: x[1], reverse=True)
        self.bid_files = [f[0] for f in self.bid_files]
        
        # Update the window title with the number of elements
        self.parent.title(f"Open Bid File [{len(self.bid_files)}]")
        
        if not self.bid_files:
            # Display a message if no files are found
            no_files_label = ttk.Label(self.thumbnails_frame, 
