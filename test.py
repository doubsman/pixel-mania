...
class ImageEditorApp(BidFile, ManageCanvas, ActionState):
    ...
    def draw_grill(self):
        if not self.bool_grid:
            ...
            #width_image = self.canvas.winfo_width()
            #height_image = self.canvas.winfo_height()
            for line in range(0, width_image, self.image_scale):
                self.canvas.create_line([(line+1, 0), (line+1, height_image)], fill='gray', dash=(1,1), tags='grid_line_w')
            for line in range(0, height_image, self.image_scale):
                self.canvas.create_line([(0, line+1), (width_image, line+1)], fill='gray', dash=(1,1), tags='grid_line_h')
            self.bool_grid = True
        else:
